/**
 * Gemini / Google API Schema Cleaner
 *
 * Strips unsupported JSON Schema keywords from tool definitions before
 * sending to Google/Gemini APIs. Adapted from OpenClaw's clean-for-gemini.ts.
 *
 * References:
...
/**
 * JSON Schema keywords that Gemini/Google APIs reject.
 * Cloud Code Assist API validates tool schemas more strictly than draft 2020-12.
 */
export const GEMINI_UNSUPPORTED_SCHEMA_KEYWORDS = new Set([
  ...
  "$defs",
  "definitions",
  // Non-standard (OpenAPI) keyword; Google validators reject it.
  "examples",

  // Validation constraints that trigger 400s on Gemini.
  "minLength",
  "maxLength",
  "minimum",
  "maximum",
...
function cleanSchemaForGeminiWithDefs(
  ...
  // --- Pre-clean anyOf/oneOf variants ---
  ...
  for (const [key, value] of Object.entries(obj)) {
    ...
    // Strip type when union variants are present
    if (key === "type" && (hasAnyOf || hasOneOf)) {
      continue;
    }
    // Normalize array types by stripping null
    if (
      key === "type" &&
      Array.isArray(value) &&
    ...
    if (key === "properties") {
      if (value && typeof value === "object" && !Array.isArray(value)) {
        ...
        // Guard malformed schemas (e.g. properties: null)
        cleaned[key] = {};
      }
  ...
  // Last-resort flattening for anyOf/oneOf arrays that couldn't be simplified.
 ...
 * Entry point — resolves $defs and recursively strips unsupported keywords.
 */
